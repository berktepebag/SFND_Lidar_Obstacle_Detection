...
std::unordered_set<int> RansacPlane(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	for (size_t i = 0; i < maxIterations; i++)
	{
		...
		A = ((point2.y-point1.y)*(point2.z-point1.z)-(point2.z-point1.z)*(point3.y-point1.y));
		...
		std::cout << "Plane: " << A << "x + " << B <<" y + " <<" z + " << C << " + " << D << std::endl;
		std::cout << "****************************************" << std::endl;
		...
		{			
			...
			distance = (A*cloud->points[j].x+B*cloud->points[j].y+C*cloud->points[j].z+D)/sqrt(A*A+B*B+C*C);		
			//std::cout << "distance: " << distance << std::endl;
			// If distance is smaller than threshold count it as inlier
			if(fabs(distance) <= distanceTol){
					inliers.insert(j);			
